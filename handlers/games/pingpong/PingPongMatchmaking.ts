...
// @/server/handlers/games/pingpong/PingPongMatchmaking.ts
 ...
 */
// =====================================================================
interface UpdateHostPreferencePayload {
  color: "red" | "blue";
}
 ...
 */
// =====================================================================
export class PingPongMatchmaking extends BaseMatchmaking {
  /**
  ...
  registerHandlers(): void {
    ...
    // 방장 색상 선택 변경 (대기 중)
    this.socket.on(
      "pingpong:update_host_preference",
      (payload: UpdateHostPreferencePayload) =>
        this.handleUpdateHostPreference(payload)
    );
  }
   ...
   */
  // =====================================================================
  private handleUpdateHostPreference(
    payload: UpdateHostPreferencePayload
  ): void {
    const { color } = payload;
    ...
    if (room) {
      // 타입 확장: hostColor 속성 추가
      (room as ServerRoom & { hostColor?: "red" | "blue" }).hostColor = color;
      console.log(
        `[핑퐁][대기방] 방장(${this.socket.id}) 색상 변경 → ${color}`
      );
    }
  ...
  protected assignRoles(room: ServerRoom): void {
    // 타입 확장을 통해 hostColor 접근
    const roomWithColor = room as ServerRoom & { hostColor?: "red" | "blue" };

    // 방장 기본값: RED(Left)
    const hostColor = roomWithColor.hostColor || "red";
    const hostSide = hostColor === "red" ? "left" : "right";
