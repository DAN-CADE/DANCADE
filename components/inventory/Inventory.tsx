...
export default function Inventory() {
  ...
  useEffect(() => {
    ...
    // 인벤토리 대각선 길이 (가장 긴 거리)
    const inventoryDiagonal = Math.sqrt(inventoryWidth * inventoryWidth + inventoryHeight * inventoryHeight);
    // 캐릭터 대각선 길이
    const characterDiagonal = Math.sqrt(characterWidth * characterWidth + characterHeight * characterHeight);
    ...
    const updatePosition = () => {
      try {
        ...
        if (gameContainer) {
          ...
          if (finalX + inventoryWidth > windowWidth) finalX = windowWidth - inventoryWidth - 10;
          if (finalY < 0) finalY = 10;
          if (finalY + inventoryHeight > windowHeight) finalY = windowHeight - inventoryHeight - 10;
          ...
          if (isOverlapping || distanceFromPlayer < minDistance) {
            ...
            const newCenterX = playerCenterX + Math.cos(angleToPlayer) * minDistance;
            const newCenterY = playerCenterY + Math.sin(angleToPlayer) * minDistance;
            ...
            if (adjustedDistance < minDistance * 0.9) {
              ...
              const newCenterX2 = playerCenterX + Math.cos(oppositeAngle) * minDistance;
              const newCenterY2 = playerCenterY + Math.sin(oppositeAngle) * minDistance;
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (adjustedX + inventoryWidth > windowWidth) adjustedX = windowWidth - inventoryWidth - 10;
      if (adjustedY < 0) adjustedY = 10;
      if (adjustedY + inventoryHeight > windowHeight) adjustedY = windowHeight - inventoryHeight - 10;
...
function isColorEquipped(
  ...
  return (
    <div 
      className={`fixed w-[400px] h-[520px] bg-[rgba(10,15,30,0.85)] border border-white/20 rounded-lg backdrop-blur z-50 flex flex-col ${isDragging ? 'cursor-move' : ''}`}
      style={style}
    >
      {/* ================= Header ================= */}
      <div 
